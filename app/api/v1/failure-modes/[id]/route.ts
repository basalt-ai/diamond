...
export const GET = withApiMiddleware(async (_req: NextRequest, ctx) => {
  ...
  const result = await manageFailureModes.get(parseUUID(id));
  return ok(result);
});
...
export const PUT = withApiMiddleware(async (req: NextRequest, ctx) => {
  ...
  const result = await manageFailureModes.update(parseUUID(id), input);
  return ok(result);
});

export const DELETE = withApiMiddleware(async (_req: NextRequest, ctx) => {
  const { id } = await ctx.params;
  await manageFailureModes.delete(parseUUID(id));
  return noContent();
});
