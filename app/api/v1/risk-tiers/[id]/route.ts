...
export const GET = withApiMiddleware(async (_req: NextRequest, ctx) => {
  ...
  const result = await manageRiskTiers.get(parseUUID(id));
  return ok(result);
});
...
export const PUT = withApiMiddleware(async (req: NextRequest, ctx) => {
  ...
  const result = await manageRiskTiers.update(parseUUID(id), input);
  return ok(result);
});

export const DELETE = withApiMiddleware(async (_req: NextRequest, ctx) => {
  const { id } = await ctx.params;
  await manageRiskTiers.delete(parseUUID(id));
  return noContent();
});
